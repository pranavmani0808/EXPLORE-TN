...
class StructuredTripIntent(BaseModel):
    ...
    durationDays: int = Field(default=3, description="Requested duration in days")
...
class IntentExtractor:
    ...
    def extract_intent(cls, text: str, current_state: Optional[dict] = None) -> StructuredTripIntent:
        ...
        duration = current.get("durationDays", 3 if ("arupadai" in lower or "six murugan" in lower or "6 murugan" in lower) else 1)
        ...
        # 1. Curated Trail Recognition (Arupadai Veedu)
        if any(kw in lower for kw in ["arupadai", "six murugan", "6 murugan", "six abodes", "murugan temples", "arupadai veedu"]):
            trail = "arupadai-veedu"
            interests.add("spiritual")
            interests.add("temple")

            # Auto-populate the six canonical temples if not already in waypoints
            if not destination or destination not in cls.ARUPADAI_VEEDU_TEMPLES:
                destination = cls.ARUPADAI_VEEDU_TEMPLES[-1] # Thirupparankundram Murugan Temple
            
            for temple in cls.ARUPADAI_VEEDU_TEMPLES[:-1]:
                if temple not in waypoints:
                    waypoints.append(temple)

        # 2. Departure Time & Overnight Parsing
        ...
        # 3. Waypoint Extraction (e.g., "through madurai", "via salem")
        ...
        # 4. Origin & Destination Extraction Patterns
            ...
            if pattern_b:
                ...
                if pattern_c:
                    ...
                    if len(raw_orig) > 2 and raw_orig.lower() not in ["a", "the", "trip", "plan"]:
                        origin = raw_orig
                    if len(raw_dest) > 2 and raw_dest.lower() not in ["a", "the", "somewhere"] and not trail:
                        destination = raw_dest

        if waypoints and destination in waypoints:
            waypoints.remove(destination)

        # 5. Duration Days Extraction
        ...
        return StructuredTripIntent(
            ...
            destination=destination or "Thirupparankundram Murugan Temple",
            waypoints=waypoints,
            trail=trail,
